// TalkBook — Variation B · Asimétrico editorial

function TBBlockB({ children, num, title }) {
  return (
...
function TBStackMatrixB({ stack, lang }) {
...
function TBFlowDiagramB({ lang }) {
  const L = lang === "es" ? {
    a: "Cliente", aSub: "Expo · React Native",
    b: "Edge", bSub: "Hono sobre Cloudflare Workers",
    c: "Postgres", cSub: "Supabase · RLS",
    d: "OpenAI", dSub: "GPT-4 mini · streaming",
    e: "Google Books", eSub: "Metadatos + portadas",
    f: "MMKV", fSub: "Cache offline en cliente",
  } : {
    a: "Client", aSub: "Expo · React Native",
    b: "Edge", bSub: "Hono on Cloudflare Workers",
    c: "Postgres", cSub: "Supabase · RLS",
    d: "OpenAI", dSub: "GPT-4 mini · streaming",
    e: "Google Books", eSub: "Metadata + covers",
    f: "MMKV", fSub: "Client-side offline cache",
  };
  ...
  return (
    <div style={{
      position: "relative",
      border: "1px solid var(--border)",
      borderRadius: 12, padding: 28, marginTop: 8,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 60, marginTop: 28, position: "relative", zIndex: 1 }}>
        <Node title={L.f} sub={L.fSub}/>
        <div/>
        <Node title={L.e} sub={L.eSub}/>
      </div>
    </div>
  );
}

function TBVariationB({ lang, setLang, theme, setTheme }) {
  const t = TALKBOOK[lang];
  const [lbIndex, setLbIndex] = useState(null);

  const tiles = useMemo(() => ([
    { label: "01 · Biblioteca + Chat", kind: "mock", render: () => <MockTBPair/>,    aspect: "16 / 9" },
    { label: "02 · Stats + Chats",     kind: "mock", render: () => <MockTBPair2/>,   aspect: "16 / 9" },
    { label: "03 · Mi Biblioteca",     kind: "mock", render: () => <MockTBLibrary/>, aspect: "4 / 5" },
    { label: "04 · Chat con el libro", kind: "mock", render: () => <MockTBBookChat/>,aspect: "4 / 5" },
    { label: "05 · Lector + notas",    kind: "mock", render: () => <MockTBReader/>,  aspect: "4 / 5" },
    { label: "06 · Búsqueda",          kind: "mock", render: () => <MockTBSearch/>,  aspect: "4 / 5" },
  ]), []);
    ...
    <div style={{ position: "relative", background: "var(--bg-primary)", color: "var(--text-primary)", minHeight: "100%", fontFamily: "var(--font-sans)" }}>
      ...
      <main style={{ padding: "0 64px 96px" }}>
        ...
        <section style={{
          ...
          alignItems: "end", padding: "96px 0 48px",
        }}>
          <div style={{
            gridColumn: "1 / span 1",
            writingMode: "vertical-rl", transform: "rotate(180deg)",
            fontFamily: "var(--font-mono)", fontSize: 12,
            letterSpacing: ".24em", color: "var(--text-secondary)",
            alignSelf: "start", paddingTop: 12,
          }}>2025 — 2026</div>
          ...
          <div style={{ gridColumn: "2 / span 7", display: "flex", flexDirection: "column", gap: 24 }}>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-secondary)", letterSpacing: ".12em", textTransform: "uppercase" }}>
              {lang === "es" ? "Case study · Mobile · Side project" : "Case study · Mobile · Side project"}
            </div>
            <h1 style={{
              font: "600 200px/.86 var(--font-display)",
          ...
          <div style={{ gridColumn: "10 / span 3", display: "flex", flexDirection: "column", gap: 16, alignSelf: "end", paddingBottom: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ color: "var(--text-primary)", textAlign: "right" }}>{t.role}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "var(--text-secondary)" }}>{lang === "es" ? "Plataforma" : "Platform"}</span>
                <span style={{ color: "var(--text-primary)" }}>iOS · Android</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 32, marginBottom: 24 }}>
          <div style={{ gridColumn: "2 / span 10", position: "relative", aspectRatio: "16 / 9", borderRadius: 10, overflow: "hidden", border: "1px solid var(--border)" }}>
            <img src="assets/talkbook-main.webp" alt="TalkBook" style={{ width: "100%", height: "100%", objectFit: "cover" }}/>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 32, marginBottom: 64 }}>
          <p style={{ gridColumn: "2 / span 10", fontSize: 13, lineHeight: 1.55, color: "var(--text-secondary)", margin: 0, fontStyle: "italic" }}>
            {lang === "es" ? "Tres pantallas — biblioteca, biblioteca abierta, selector de chat — de la beta de marzo." : "Three screens — library, open library, chat picker — from the March beta."}
          </p>
        </div>
        ...
        <section style={{
          ...
          <div style={{ gridColumn: "2 / span 5" }}>
            ...
            <div style={{
              ...
              1<span style={{ color: "var(--text-secondary)" }}>:</span>1
            </div>
          </div>
          <div style={{ gridColumn: "7 / span 5" }}>
            ...
            <p style={{ fontSize: 14, color: "var(--text-secondary)", marginTop: 16, lineHeight: 1.6 }}>
              {lang === "es"
                ? "Un chat por libro, con tu contexto. No otra app de stickers de estrellas."
                : "One chat per book, with your context. Not another star-sticker app."}
            </p>
          </div>
        </section>

        {/* 01 Problema */}
        <TBBlockB num={1} title={t.problemTitle}>
        ...
        {/* 02 Stack */}
        <TBBlockB num={2} title={t.stackTitle}>
          <TBStackMatrixB stack={TB_STACK[lang]} lang={lang}/>
        </TBBlockB>

        {/* 03 Features */}
        <TBBlockB num={3} title={t.featuresTitle}>
        ...
        {/* 04 Capturas */}
        <TBBlockB num={4} title={t.galleryTitle}>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 32, paddingTop: 48, borderTop: "1px solid var(--border)", marginBottom: 64 }}>
          ...
          <div style={{ gridColumn: "5 / span 8", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1, background: "var(--border)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
            {[
              { href: `https://${t.site}`, label: lang === "es" ? "Beta privada" : "Private beta", sub: t.site, icon: <ExtIcon width={14} height={14}/> },
              { href: "https://github.com/HugoOlivaR", label: "GitHub", sub: lang === "es" ? "Privado · demo" : "Private · demo", icon: <GitHubIcon width={14} height={14}/> },
              { href: "#", label: lang === "es" ? "Post relacionado" : "Related post", sub: lang === "es" ? "Construir móvil con Expo en 2026" : "Building mobile with Expo in 2026", icon: <ArrowRightIcon width={14} height={14}/> },
            ].map((l) => (
...
Object.assign(window, { TBVariationB });
